...
FROM --platform=$BUILDPLATFORM docker.io/library/node:24.4.1-bookworm-slim AS npm-builder
...
FROM --platform=$BUILDPLATFORM docker.io/library/rust:1.88.0-slim-bookworm AS rust-builder
SHELL ["/bin/bash", "-euxo", "pipefail", "-c"]
HEALTHCHECK NONE
RUN mkdir -p /root/src
WORKDIR /root/src
