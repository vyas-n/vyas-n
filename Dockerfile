...
FROM --platform=$BUILDPLATFORM docker.io/library/node:26.4.0-bookworm-slim AS npm-builder
...
FROM --platform=$BUILDPLATFORM docker.io/library/rust:1.96.1-slim-bookworm AS rust-builder
SHELL ["/bin/bash", "-euxo", "pipefail", "-c"]
HEALTHCHECK NONE
RUN mkdir -p /root/src
WORKDIR /root/src
...
FROM docker.io/joseluisq/static-web-server:2.43.0-debian
SHELL ["/bin/bash", "-euxo", "pipefail", "-c"]
HEALTHCHECK NONE
WORKDIR /
