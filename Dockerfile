...
FROM --platform=$BUILDPLATFORM docker.io/library/node:26.5.0-bookworm-slim AS npm-builder
...
FROM --platform=$BUILDPLATFORM docker.io/library/rust:1.97.0-slim-bookworm AS rust-builder
SHELL ["/bin/bash", "-euxo", "pipefail", "-c"]
HEALTHCHECK NONE
RUN mkdir -p /root/src
WORKDIR /root/src
